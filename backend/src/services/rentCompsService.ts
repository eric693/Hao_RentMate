...
export function inferDistrict(address: string): string | null {
...
let cache: { at: number; rows: RentComp[] } | null = null;
const TTL = 10 * 60 * 1000;

async function buildComps(): Promise<RentComp[]> {
  ...
  // key = district|type
  const buckets = new Map<string, { rents: number[]; perPing: number[]; district: string; type: string }>();
  for (const c of contracts) {
    ...
    if (!buckets.has(key)) buckets.set(key, { rents: [], perPing: [], district, type });
    const b = buckets.get(key)!;
    const rent = Number(c.monthlyRent);
    b.rents.push(rent);
    if (c.unit.areaPing && Number(c.unit.areaPing) > 0) b.perPing.push(rent / Number(c.unit.areaPing));
  }
...
export async function getComps(): Promise<RentComp[]> {
...
// 針對某倉庫給定價建議（與在地行情比較）
export async function pricingForUnit(unitId: string): Promise<PricingSuggestion | null> {
  ...
  if (!comp) {
    ...
    if (sameDistrict.length) {
      ...
      comp = {
        ...
        avgRent: Math.round(rents.reduce((s: number, r: number) => s + r, 0) / rents.length),
  ...
  let position: PricingSuggestion['position'] = 'INLINE';
  let advice = '';
  if (deltaPct <= -8) {
    position = 'BELOW';
