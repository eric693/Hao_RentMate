...
const roiController_1 = require("./roiController");
const dashboardController_1 = require("./dashboardController");
const collectionWorkbenchController_1 = require("./collectionWorkbenchController");
const money = (v) => (v === null || v === undefined ? '' : `NT$${Number(v).toLocaleString()}`);
const date = (v) => (v ? new Date(v).toLocaleDateString('zh-TW') : '');
const CAT = { WATER: '水費', ELECTRICITY: '電費', GAS: '瓦斯費', MANAGEMENT: '管理費', REPAIR: '維修費', INSURANCE: '保險', INTERNET: '網路', OTHER: '其他' };
const RENT_ST = { PAID: '已繳清', PENDING: '待繳', PARTIAL: '部分繳款', OVERDUE: '逾期' };
...
const DATASETS = {
    ...
    contracts: {
        ...
        fetch: async (userId) => {
            const props = await app_1.prisma.property.findMany({ where: { userId }, select: { id: true } });
            const units = await app_1.prisma.unit.findMany({ where: { propertyId: { in: props.map((p) => p.id) } }, select: { id: true } });
    ...
    payments: {
        ...
        fetch: async (userId) => {
            const contracts = await app_1.prisma.contract.findMany({ where: { unit: { property: { userId } } }, select: { id: true } });
    ...
    roi: {
        ...
        fetch: async (userId) => {
            const result = await (0, roiController_1.computeROI)(userId);
            return result.map((r) => ({
    ...
    dashboard: {
        ...
        fetch: async (userId) => {
            const d = await (0, dashboardController_1.computeDashboard)(userId);
            return [
    ...
    'finance-overview': {
        ...
        fetch: async (userId) => {
            const now = new Date();
            const f = await (0, collectionWorkbenchController_1.computeFinanceOverview)(userId, now.getFullYear(), now.getMonth() + 1);
            const rows = [
            ...
            for (const e of f.expenseBreakdown)
                rows.push({ item: `支出明細－${e.label}`, value: `${money(e.amount)}（${e.pct}%）` });
            return rows;
        },
    },
};
async function exportData(req, res) {
    ...
    if (!ds) {
        res.status(404).json({ error: '不支援的匯出類型' });
        return;
    }
    // 權限：ADMIN 放行；空模組(如總覽)任何登入者皆可；其餘 STAFF 需具該模組權限
    if (req.role !== 'ADMIN' && ds.module && !req.permissions?.includes(ds.module)) {
        res.status(403).json({ error: '您沒有匯出此資料的權限' });
        return;
    }
    const rows = await ds.fetch(req.userId);
    const { mime, disposition } = (0, exportService_1.fileResponseHeaders)(ds.title, format);
    res.setHeader('Content-Type', mime);
    res.setHeader('Content-Disposition', disposition);
    if (format === 'excel') {
        res.send((0, exportService_1.buildExcel)(ds.title, ds.columns, rows));
    }
    else {
        res.send(await (0, exportService_1.buildPdf)(ds.title, ds.columns, rows));
    }
}
