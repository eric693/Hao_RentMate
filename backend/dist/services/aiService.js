...
const MAINTENANCE_CATEGORIES = ['水電', '鐵捲門/門禁', '消防/安全', '漏水/排水', '結構/地坪', '溫控設備', '裝卸/設備', '清潔/環境', '其他'];
...
function ruleBasedClassify(title, description) {
    ...
    let category = '其他';
    if (/漏水|滲水|積水|水管|排水|淹水/.test(t))
        category = '漏水/排水';
    else if (/鐵捲門|捲門|門禁|閘門|電動門|刷卡|感應/.test(t))
        category = '鐵捲門/門禁';
    else if (/消防|滅火|警報|煙霧|逃生|安全/.test(t))
        category = '消防/安全';
    else if (/冷藏|冷凍|溫控|恆溫|製冷|壓縮機|溫度/.test(t))
        category = '溫控設備';
    else if (/堆高機|棧板|貨架|月台|裝卸|升降/.test(t))
        category = '裝卸/設備';
    else if (/停電|跳電|插座|電燈|電線|開關|配電/.test(t))
        category = '水電';
    else if (/牆|裂|天花板|地坪|地板|結構|樑柱/.test(t))
        category = '結構/地坪';
    else if (/清潔|髒|垃圾|蟲|鼠|環境/.test(t))
        category = '清潔/環境';
    let priority = 'MEDIUM';
    if (/漏水|停電|消防|火|危險|安全|鐵捲門|無法上鎖|淹水|溫控失效|冷藏故障/.test(t))
        priority = 'HIGH';
